...
gridpoint <- as(forest3,'SpatialPixelsDataFrame')%>%st_as_sf
...
v_ai<-sample_lsm(lu_classified,gridpoint,shape="square",what="lsm_c_ai",,size=5000,progress = TRUE,verbose=T)
v_ai<-subset(v_ai,class==1)


v_area_mn<-sample_lsm(lu_classified,gridpoint,shape="square",what="lsm_c_area_mn",size=5000,progress = TRUE)
v_area_mn<-subset(v_area_mn,class==1)

v_ed<-sample_lsm(lu_classified,gridpoint,shape="square",what="lsm_c_ai",size=5000,progress = TRUE)
...
for (i in 1:nrow(gridpoint_ai)){
  
  if (any(v_ai$plot_id==i)){
    gridpoint_ai[i,'layer']<-as.numeric(v_ai[v_ai$plot_id==i,6])
  }
  
  else{
    gridpoint_ai[i,'layer']<-NA
  }
}


for (i in 1:nrow(gridpoint_area_mn)){
  
  if (any(v_area_mn$plot_id==i)){
    gridpoint_area_mn[i,'layer']<-as.numeric(v_area_mn[v_area_mn$plot_id==i,6])
  }
  
  else{
    gridpoint_area_mn[i,'layer']<-NA
  }
}



for (i in 1:nrow(gridpoint_ed)){
  
  if (any(v_ed$plot_id==i)){
    gridpoint_ed[i,'layer']<-as.numeric(v_ed[v_ed$plot_id==i,6])
  }
  
  else{
    gridpoint_ed[i,'layer']<-NA
  }
}
